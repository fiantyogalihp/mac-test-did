...
#include <intrin.h>
	...
	void getMacHash(u16 &mac1, u16 &mac2)
	{
		...
		{
			delete[] AdapterInfo; // Cleanup in case of error
			return;								// no adapters.
		}
		...
		delete[] AdapterInfo; // Cleanup after use
	}
...
#include <mach-o/arch.h>
	...
	std::string generateHash(const std::string &bytes)
	{
		...
		{
			unsigned char ch = static_cast<unsigned char>(~((static_cast<unsigned char>(bytes[i]) +
																											 static_cast<unsigned char>(bytes[(static_cast<unsigned long long>(i) + 1) % size]) +
																											 static_cast<unsigned char>(bytes[(static_cast<unsigned long long>(i) + 2) % size]) +
																											 static_cast<unsigned char>(bytes[(static_cast<unsigned long long>(i) + 3) % size]))) *
																										(i + 1));
